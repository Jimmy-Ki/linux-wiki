---
title: gpasswd - Administer /etc/group and /etc/gshadow
sidebar_label: gpasswd
---



> **Command documentation sourced from the linux-command project**
> _This comprehensive command reference is part of the linux-command documentation project._



# gpasswd - Administer /etc/group and /etc/gshadow



## Overview

The `gpasswd` command administer /etc/group and /etc/gshadow.

## Syntax

```bash
gpasswd [OPTIONS] [ARGUMENTS]
```

## Basic Usage Examples

```bash
# Basic usage
gpasswd

# Get help
gpasswd --help

# Check version
gpasswd --version
```

## Common Options

- `-h, --help` - Display help information
- `-v, --version` - Show version information
- `-V, --verbose` - Enable verbose output (if supported)

## Description

The `gpasswd` command is part of the `user-management` category in Linux systems. It provides essential functionality for administer /etc/group and /etc/gshadow.

Refer to the manual page (`man gpasswd`) for detailed information about all available options and usage patterns.

## Common Use Cases

1. **Basic Operations**: Simple usage scenarios
2. **System Administration**: Administrative tasks
3. **Automation**: Script integration
4. **Troubleshooting**: Diagnostic usage

## Related Commands

- Other commands in the `user-management` category
- System utilities for similar operations