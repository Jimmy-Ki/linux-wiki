...
The `arpwatch` command arp monitoring tool.

## Syntax

```bash
arpwatch [OPTIONS] [ARGUMENTS]
```
...
The `arpwatch` command is part of the `system-info` category in Linux systems. It provides essential functionality for arp monitoring tool.

Refer to the manual page (`man arpwatch`) for detailed information about all available options and usage patterns.
