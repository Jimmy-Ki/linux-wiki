---
title: arping - Send ARP REQUEST to a host
sidebar_label: arping
---



> **Command documentation sourced from the linux-command project**
> _This comprehensive command reference is part of the linux-command documentation project._



# arping - Send ARP REQUEST to a host



## Overview

The `arping` command send arp request to a host.

## Syntax

```bash
arping [OPTIONS] [ARGUMENTS]
```

## Basic Usage Examples

```bash
# Basic usage
arping

# Get help
arping --help

# Check version
arping --version
```

## Common Options

- `-h, --help` - Display help information
- `-v, --version` - Show version information
- `-V, --verbose` - Enable verbose output (if supported)

## Description

The `arping` command is part of the `system-info` category in Linux systems. It provides essential functionality for send arp request to a host.

Refer to the manual page (`man arping`) for detailed information about all available options and usage patterns.

## Common Use Cases

1. **Basic Operations**: Simple usage scenarios
2. **System Administration**: Administrative tasks
3. **Automation**: Script integration
4. **Troubleshooting**: Diagnostic usage

## Related Commands

- Other commands in the `system-info` category
- System utilities for similar operations