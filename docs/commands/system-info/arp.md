---
title: arp - Display and manipulate ARP cache
sidebar_label: arp
---



> **Command documentation sourced from the linux-command project**
> _This comprehensive command reference is part of the linux-command documentation project._



# arp - Display and manipulate ARP cache



## Overview

The `arp` command display and manipulate arp cache.

## Syntax

```bash
arp [OPTIONS] [ARGUMENTS]
```

## Basic Usage Examples

```bash
# Basic usage
arp

# Get help
arp --help

# Check version
arp --version
```

## Common Options

- `-h, --help` - Display help information
- `-v, --version` - Show version information
- `-V, --verbose` - Enable verbose output (if supported)

## Description

The `arp` command is part of the `system-info` category in Linux systems. It provides essential functionality for display and manipulate arp cache.

Refer to the manual page (`man arp`) for detailed information about all available options and usage patterns.

## Common Use Cases

1. **Basic Operations**: Simple usage scenarios
2. **System Administration**: Administrative tasks
3. **Automation**: Script integration
4. **Troubleshooting**: Diagnostic usage

## Related Commands

- Other commands in the `system-info` category
- System utilities for similar operations