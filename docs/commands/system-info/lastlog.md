---
title: lastlog - Reports the most recent login of all users
sidebar_label: lastlog
---



> **Command documentation sourced from the linux-command project**
> _This comprehensive command reference is part of the linux-command documentation project._



# lastlog - Reports the most recent login of all users



## Overview

The `lastlog` command reports the most recent login of all users.

## Syntax

```bash
lastlog [OPTIONS] [ARGUMENTS]
```

## Basic Usage Examples

```bash
# Basic usage
lastlog

# Get help
lastlog --help

# Check version
lastlog --version
```

## Common Options

- `-h, --help` - Display help information
- `-v, --version` - Show version information
- `-V, --verbose` - Enable verbose output (if supported)

## Description

The `lastlog` command is part of the `system-info` category in Linux systems. It provides essential functionality for reports the most recent login of all users.

Refer to the manual page (`man lastlog`) for detailed information about all available options and usage patterns.

## Common Use Cases

1. **Basic Operations**: Simple usage scenarios
2. **System Administration**: Administrative tasks
3. **Automation**: Script integration
4. **Troubleshooting**: Diagnostic usage

## Related Commands

- Other commands in the `system-info` category
- System utilities for similar operations