---
title: php - PHP command-line interface
sidebar_label: php
---



> **Command documentation sourced from the linux-command project**
> _This comprehensive command reference is part of the linux-command documentation project._



# php - PHP command-line interface



## Overview

The `php` command php command-line interface.

## Syntax

```bash
php [OPTIONS] [ARGUMENTS]
```

## Basic Usage Examples

```bash
# Basic usage
php

# Get help
php --help

# Check version
php --version
```

## Common Options

- `-h, --help` - Display help information
- `-v, --version` - Show version information
- `-V, --verbose` - Enable verbose output (if supported)

## Description

The `php` command is part of the `file-management` category in Linux systems. It provides essential functionality for php command-line interface.

Refer to the manual page (`man php`) for detailed information about all available options and usage patterns.

## Common Use Cases

1. **Basic Operations**: Simple usage scenarios
2. **System Administration**: Administrative tasks
3. **Automation**: Script integration
4. **Troubleshooting**: Diagnostic usage

## Related Commands

- Other commands in the `file-management` category
- System utilities for similar operations