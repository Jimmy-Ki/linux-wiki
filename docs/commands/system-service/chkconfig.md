---
title: chkconfig - Manage system services
sidebar_label: chkconfig
---



> **Command documentation sourced from the linux-command project**
> _This comprehensive command reference is part of the linux-command documentation project._



# chkconfig - Manage system services



## Overview

The `chkconfig` command manage system services.

## Syntax

```bash
chkconfig [OPTIONS] [ARGUMENTS]
```

## Basic Usage Examples

```bash
# Basic usage
chkconfig

# Get help
chkconfig --help

# Check version
chkconfig --version
```

## Common Options

- `-h, --help` - Display help information
- `-v, --version` - Show version information
- `-V, --verbose` - Enable verbose output (if supported)

## Description

The `chkconfig` command is part of the `system-service` category in Linux systems. It provides essential functionality for manage system services.

Refer to the manual page (`man chkconfig`) for detailed information about all available options and usage patterns.

## Common Use Cases

1. **Basic Operations**: Simple usage scenarios
2. **System Administration**: Administrative tasks
3. **Automation**: Script integration
4. **Troubleshooting**: Diagnostic usage

## Related Commands

- Other commands in the `system-service` category
- System utilities for similar operations