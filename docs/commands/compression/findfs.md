---
title: findfs - Find filesystem by label or UUID
sidebar_label: findfs
---



> **Command documentation sourced from the linux-command project**
> _This comprehensive command reference is part of the linux-command documentation project._



# findfs - Find filesystem by label or UUID



## Overview

The `findfs` command find filesystem by label or uuid.

## Syntax

```bash
findfs [OPTIONS] [ARGUMENTS]
```

## Basic Usage Examples

```bash
# Basic usage
findfs

# Get help
findfs --help

# Check version
findfs --version
```

## Common Options

- `-h, --help` - Display help information
- `-v, --version` - Show version information
- `-V, --verbose` - Enable verbose output (if supported)

## Description

The `findfs` command is part of the `compression` category in Linux systems. It provides essential functionality for find filesystem by label or uuid.

Refer to the manual page (`man findfs`) for detailed information about all available options and usage patterns.

## Common Use Cases

1. **Basic Operations**: Simple usage scenarios
2. **System Administration**: Administrative tasks
3. **Automation**: Script integration
4. **Troubleshooting**: Diagnostic usage

## Related Commands

- Other commands in the `compression` category
- System utilities for similar operations