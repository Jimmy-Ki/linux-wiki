...
export const SHA1Tool: React.FC = () => {
  ...
  const handleHash = useCallback(async () => {
    ...
    try {
      ...
      if (state.inputFile) {
        // Hash file
        hash = await HashingUtils.hashFileSHA1(state.inputFile, (progress) => {
          setState(prev => ({ ...prev, progress }));
        });
      } else {
        // Hash text
        hash = await HashingUtils.hashSHA1(state.input);
  ...
  const handleDownload = useCallback(() => {
    ...
    a.href = url;
    a.download = `sha1-${state.selectedFormat}-${state.inputFileName || 'hash'}.txt`;
  ...
  const downloadBatchResults = useCallback(() => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `sha1-batch-results-${Date.now()}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  return (
    <div className="crypto-tool">
      <div className="crypto-tool-header">
        <h3>SHA-1 Hash Calculator</h3>
        <p>
          Calculate SHA-1 hashes for text and files with support for hex, base64, and base64url formats.
          Handles files up to 100MB with progress tracking and batch processing capabilities.
          <strong>Note: SHA-1 is deprecated for security applications but remains useful for compatibility and non-security purposes.</strong>
        </p>
      ...
      <div className="crypto-tool-body">
        ...
        <div className="crypto-tool-section">
          ...
          <textarea
            ...
            placeholder="Enter text to hash with SHA-1..."
        ...
        {state.hashResults.hex && (
          <div className="crypto-tool-section success">
            <div className="section-header">
              <h4>
                SHA-1 Hash ({state.selectedFormat.toUpperCase()})
