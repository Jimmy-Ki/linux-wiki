...
export const SHA256Tool: React.FC = () => {
  ...
  const handleHash = useCallback(async () => {
    ...
    try {
      ...
      if (state.inputFile) {
        // Hash file
        hash = await HashingUtils.hashFileSHA256(state.inputFile, (progress) => {
          setState(prev => ({ ...prev, progress }));
        });
      } else {
        // Hash text
        hash = await HashingUtils.hashSHA256(state.input);
  ...
  const handleDownload = useCallback(() => {
    ...
    a.href = url;
    a.download = `sha256-${state.selectedFormat}-${state.inputFileName || 'hash'}.txt`;
  ...
  const downloadBatchResults = useCallback(() => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `sha256-batch-results-${Date.now()}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  return (
    <div className="crypto-tool">
      <div className="crypto-tool-header">
        ...
        <p>
          Calculate SHA-256 hashes for text and files with support for hex, base64, and base64url formats.
          Handles files up to 100MB with progress tracking and batch processing capabilities.
          Uses the Web Crypto API for optimal performance and security.
        </p>
      ...
      <div className="crypto-tool-body">
        ...
        {state.hashResults.hex && (
          <div className="crypto-tool-section success">
            <div className="section-header">
              <h4>
                SHA-256 Hash ({state.selectedFormat.toUpperCase()})
